...
--
-- This is an increment on top of the 'Initial.Typechecker' module,
-- that includes the 'Reader' monad to remove some of the boilerplate.
--
...
-- | Environment generation from an AST program.
genEnv :: Program -> Env
...
instance Typecheckable MethodDef where
  typecheck mdef@(MethodDef {mname, mparams, mbody, mtype}) = do
